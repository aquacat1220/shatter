        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            if let Some(body_to_edit) = self.body_to_edit {
                if let Ok(mut body_to_edit) = self.world.body_mut(body_to_edit) {
                    let body_screen_pos = screen_center
                        + Self::world_delta_to_screen(
                    ...
                    let response = ui.allocate_rect(gizmo_rect, sense);
                    ...
                    let mut pos_controller_color =
                        ctx.style().visuals.gray_out(egui::Color32::YELLOW);
